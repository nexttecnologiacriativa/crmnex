...
export default function NativeConversationInterface() {
  ...
  const { data: conversations = [], refetch: refetchConversations } = useQuery({
    queryKey: ['whatsapp-conversations-native', currentWorkspace?.id],
    queryFn: async () => {
  ...
  const { data: messages = [], refetch: refetchMessages } = useQuery({
    queryKey: ['whatsapp-messages-native', selectedConversation?.id],
    queryFn: async () => {
  ...
  const handleAudioSent = async (audioData: string) => {
    if (!selectedConversation) return;
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {selectedConversation.leads?.company && `${selectedConversation.leads.company} • `}
                      {selectedConversation.phone_number}
                    </p>
                  </div>
                </div>
