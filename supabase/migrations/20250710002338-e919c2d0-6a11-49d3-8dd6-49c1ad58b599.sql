...
keep_ids AS (
  SELECT DISTINCT ON (wm.user_id, wm.workspace_id) wm.id
  FROM workspace_members wm
  INNER JOIN duplicates d ON d.user_id = wm.user_id AND d.workspace_id = wm.workspace_id AND d.keep_date = wm.joined_at
  ORDER BY wm.user_id, wm.workspace_id, wm.id
)
...
) AND wm.id NOT IN (SELECT id FROM keep_ids);
