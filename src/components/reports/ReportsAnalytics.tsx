...
import { TrendingUp, Users, Target, DollarSign, Calendar as CalendarIcon, BarChart, Banknote, Clock, MessageCircle, UserX, AlertTriangle } from 'lucide-react';
...
function ResponseTimeSection() {
  ...
  return (
    <Card className="border-0 shadow-lg">
      <CardHeader className="bg-gradient-to-r from-nexcrm-blue/10 to-nexcrm-green/10">
        <CardTitle className="flex items-center gap-2 text-nexcrm-blue">
          <Clock className="h-5 w-5" />
          Análise de Tempo de Atendimento
        </CardTitle>
      </CardHeader>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-gradient-to-br from-red-500/10 to-red-500/5 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm text-muted-foreground">Nunca Contatados</span>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {idleData.length > 0 && (
            <div className="border rounded-lg p-4">
              <h4 className="text-sm font-medium mb-4">Distribuição por Tempo Parado</h4>
                ...
                <div className="flex-1 space-y-3">
                  {idleData.map((item, index) => (
                    <div key={index} className="flex items-center justify-between">
                      ...
                      <Badge variant="outline">{item.value} leads</Badge>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="border rounded-lg p-4">
            <h4 className="text-sm font-medium mb-4">Leads Parados (Top 10)</h4>
            {metrics.idleLeadsList.length > 0 ? (
              <div className="space-y-2 max-h-[240px] overflow-y-auto">
                {metrics.idleLeadsList.slice(0, 10).map((lead, index) => (
                  <div 
                    key={lead.id} 
                  ...
                  >
                    ...
                    <Badge 
                      variant={lead.daysIdle >= 7 ? 'destructive' : lead.daysIdle >= 4 ? 'default' : 'secondary'}
                      className="ml-2 shrink-0"
                    >
                      {lead.daysIdle}d parado
                    </Badge>
                  </div>
                ))}
              </div>
              ...
              <p className="text-sm text-muted-foreground text-center py-8">
                Nenhum lead parado há mais de 3 dias
              </p>
            )}
          </div>
        </div>
