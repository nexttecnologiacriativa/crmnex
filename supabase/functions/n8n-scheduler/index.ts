...
Deno.serve(async (req) => {
  ...
  try {
    console.log('🚀 N8n Scheduler iniciado - processamento automático');
    ...
    // Buscar itens pendentes na fila de automação para n8n
    ...
    if (!queueItems || queueItems.length === 0) {
      console.log('ℹ️ Nenhuma notificação N8N pendente na fila');
      return new Response(
        JSON.stringify({ success: true, processed: 0, status: 'no_items' }),
        { 
          status: 200,
    ...
    console.log(`📋 Processando automaticamente ${queueItems.length} notificações N8N`);
    ...
    for (const item of queueItems) {
      try {
        ...
        if (!webhooks || webhooks.length === 0) {
          console.log(`ℹ️ Nenhum webhook N8N ativo para workspace ${workspaceId}, pipeline ${pipelineId}`);
        ...
        // Preparar dados para n8n com processamento de telefone
        ...
        console.log(`📤 Enviando lead automaticamente para N8N: ${lead.name} - ${leadDataForN8n.phone}`);
        ...
        for (const webhook of webhooks) {
          try {
            console.log(`📤 Enviando para webhook N8N: ${webhook.name}`);
            ...
            if (!response.ok) {
              console.error(`❌ Webhook N8N ${webhook.name} falhou:`, response.status, response.statusText);
              allSuccessful = false;
              errors.push(`${webhook.name}: ${response.status}`);
            } else {
              console.log(`✅ Enviado com sucesso para webhook N8N: ${webhook.name}`);
            }
          } catch (error) {
            console.error(`❌ Erro enviando para webhook N8N ${webhook.name}:`, error);
            allSuccessful = false;
        ...
        console.error(`❌ Erro processando item da fila ${item.id}:`, error);
    ...
    console.log(`✅ Processamento automático concluído: ${processedCount} sucessos, ${errorCount} erros`);
    ...
    console.error('❌ Error no scheduler N8N:', error);
