-- Fix: function round(double precision, integer) does not exist
-- PostgreSQL ROUND() with 2 arguments requires numeric type, not double precision

CREATE OR REPLACE FUNCTION public.get_whatsapp_response_metrics(p_workspace_id uuid, p_days_back integer DEFAULT 30)
 RETURNS TABLE(pairs_total integer, avg_minutes numeric, median_minutes numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  stats AS (
    SELECT 
      ...
      ROUND(AVG(response_minutes)::numeric, 2) as avg_min,
      ROUND((PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY response_minutes))::numeric, 2) as median_min
    FROM valid_pairs
  )
  SELECT 
    COALESCE(total, 0),
