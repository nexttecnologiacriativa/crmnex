...
CREATE TABLE public.debriefings (
  ...
  net_revenue_calculated numeric DEFAULT 
  CASE
      WHEN (gross_revenue IS NOT NULL) THEN (gross_revenue - COALESCE(total_investment, (0)::numeric))
      ELSE NULL::numeric
  END,
...
CREATE TABLE public.debriefing_pages (
  ...
  conversion_rate numeric DEFAULT 
  CASE
      WHEN (total_views > 0) THEN (((conversions)::numeric / (total_views)::numeric) * (100)::numeric)
      ELSE (0)::numeric
  END,
...
CREATE TABLE public.debriefing_checkouts (
  ...
  conversion_rate numeric DEFAULT 
  CASE
      WHEN (total_views > 0) THEN (((completed_purchases)::numeric / (total_views)::numeric) * (100)::numeric)
      ELSE (0)::numeric
  END,
  abandonment_rate numeric DEFAULT 
  CASE
      WHEN (checkout_starts > 0) THEN (((checkout_abandonments)::numeric / (checkout_starts)::numeric) * (100)::numeric)
      ELSE (0)::numeric
  END,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
...
CREATE TABLE public.debriefing_products (
  ...
  total_revenue numeric DEFAULT (unit_price * (quantity_sold)::numeric),
  created_at timestamp with time zone NOT NULL DEFAULT now(),
